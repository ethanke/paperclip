...
export function parseKimiJsonl(stdout: string) {
...
const KIMI_AUTH_REQUIRED_RE = /(?:not\s+authenticated|please\s+authenticate|api[_ ]?key\s+(?:required|missing|invalid)|authentication\s+required|unauthorized|invalid\s+credentials|not\s+logged\s+in|login\s+required|run\s+`?kimi\s+auth(?:\s+login)?`?\s+first)/i;
const KIMI_QUOTA_EXHAUSTED_RE =
...
export function detectKimiAuthRequired(input: {
  ...
  const errors = extractKimiErrorMessages(input.parsed ?? {});
  ...
  const requiresAuth = messages.some((line) => KIMI_AUTH_REQUIRED_RE.test(line));
  return { requiresAuth };
}

export function detectKimiQuotaExhausted(input: {
  parsed: Record<string, unknown> | null;
  stdout: string;
  stderr: string;
}): { exhausted: boolean } {
  ...
  const exhausted = messages.some((line) => KIMI_QUOTA_EXHAUSTED_RE.test(line));
  return { exhausted };
}

export function isKimiTurnLimitResult(
