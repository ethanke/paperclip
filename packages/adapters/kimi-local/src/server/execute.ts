...
/**
 * Inject Paperclip skills directly into `~/.kimi/skills/` via symlinks.
 * This avoids needing KIMI_CLI_HOME overrides, so the CLI naturally finds
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const commandNotes = (() => {
    const notes: string[] = ["Prompt is passed to Kimi as the positional argument for non-interactive --print execution."];
